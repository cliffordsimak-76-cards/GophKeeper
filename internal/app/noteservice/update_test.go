...
func Test_Update(t *testing.T) {
	...
	t.Run("repository error", func(t *testing.T) {
		...
		te.noteRepoMock.EXPECT().Update(te.ctx, note).
			Return(nil, errAny)
	...
	t.Run("success", func(t *testing.T) {
		...
		te.noteRepoMock.EXPECT().Update(te.ctx, adapters.UpdateNoteRequestFromPb(req, userID)).
			Return(note, nil)

		response, err := te.service.UpdateNote(te.ctx, req)
